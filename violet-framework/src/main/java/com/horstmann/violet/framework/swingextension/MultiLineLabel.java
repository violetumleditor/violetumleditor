...
public class MultiLineLabel extends Canvas {
    ...
    public void paint(Graphics g) {
        int x, y;
        Dimension d = getSize();
        final AttributedString[] attributedStrings = new AttributedString[num_lines];
        ...
        for (int i = 0; i < num_lines; i++, y += line_height) {
            ...
            g.setFont(getFont());
            if (underline) {
                attributedStrings[i] = new AttributedString(lines[i]);
                attributedStrings[i].addAttribute(TextAttribute.UNDERLINE, TextAttribute.UNDERLINE_ON);
                g.drawString(attributedStrings[i].getIterator(), x, y);
            } else {
